...
QEMU ?= $(INFERRED_QEMU)
QEMUOPT	= -net none -parallel file:log.txt -serial pipe:./$(FIFO)
...
# Delete the build
clean:
	$(call run,rm -rfv $(DEPSDIR) $(OBJDIR) *.img core *.core $(FIFO).*,CLEAN)

realclean: clean
	$(call run,rm -rfv $(DISTDIR)-$(USER).tar.gz $(DISTDIR)-$(USER))
