...
/////////////////////////////////////////////////////////////////////////////////////////////
//moves the break of process to address, rounded up to the nearest address that is a multiple of PAGESIZE
int change_break(proc *process, uintptr_t address){
    //we need to do some checks: namely if the address is 1. past the top of the stack, 2, if the address is below the base of the heap. 
    if (address > (uintptr_t) (MEMSIZE_VIRTUAL - PAGESIZE))//address past top of stack
    {
        return -1;
    }

    else if (address < process->original_break) //address below base of heap
    {
        return -1;
    }

    else{
        //find the smallest multiple of pagesize that is larger than the input address. save as aligned_address
        uintptr_t aligned_address = (uintptr_t) NULL;

        if ((address % PAGESIZE) == 0) //address is aligned already
        {
            aligned_address = address;
        }

        else // need to round up to a mutiple of pagesize. 
        {
            aligned_address = ROUNDUP(address, PAGESIZE);
        }


        //set break to aligned address. return 0 if sucessful. -1 if failure
        if (aligned_address != (uintptr_t) NULL)
        {
            if (aligned_address > (uintptr_t)(MEMSIZE_VIRTUAL - PAGESIZE)) //check if the new aligned address is larger than the top of the stack. 
            {
                return -1;
            }

            process->program_break = aligned_address;

            //log_printf("old: %X, new: %X\n", current->original_break, process->program_break);

            return 0;
        }

        else{ //aligned_address was null
            return -1;
        }
    }
}
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_BRK:
	{
        //set break of the current process to the address passed. Do this with helpefunction change_break(proc* current, void* address)
        uintptr_t old_address = current->program_break;
        uintptr_t address = (uintptr_t) current->p_registers.reg_rdi;
        int change_break_status = change_break(current, address);

        if (change_break_status < 0)
        {
            current->p_registers.reg_rax = (uint64_t) -1;
            break;
        }
        current->p_registers.reg_rax = old_address;
	    break;
	}

    case INT_SYS_SBRK:
	{
	    //address will be current->process_break + the value passed to sbrk which will be stored in the rdi register
        uintptr_t old_address = current->program_break;
        uintptr_t address = (intptr_t) current->p_registers.reg_rdi + current->program_break;
        int change_break_status = change_break(current, address);
    ...
    case INT_PAGEFAULT: 
	{
        ...
        //page fault cuz we didn't allocate the page. Allocate the page now on the heap. 
        //first we find an availiable page in physical memory. 
        uintptr_t physical_page = next_avail_page();
        int assign_physical_result;
        int virtual_map_result;

        if (physical_page == 0) //couldn't find a physical page
        {
            syscall_exit();
            break;
        }
            ...
            //assign owner to the physical page and virtual memory map addr to the physcical page.
            assign_physical_result = assign_physical_page(physical_page, current->p_pid);
            
            virtual_map_result = virtual_memory_map(current->p_pagetable, addr, physical_page, PAGESIZE, PTE_P | PTE_W | PTE_U, NULL);
            //log_printf(" va: %X, pa %X\n", ROUNDDOWN(addr, PAGESIZE), PAGEADDRESS(physical_page));
            //log_printf(" va: %X", (MEMSIZE_VIRTUAL - PAGESIZE));
        }

        if (assign_physical_result == -1 || virtual_map_result == -1)
        {
