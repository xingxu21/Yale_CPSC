...
def test_game(count, depth, p_random, p1_h, n):
    ...
    win_pct = compare_strategies(board, lambda: p1_strategy_fxn(depth, h1), lambda: p2_strategy_fxn(n), count, 1.0 - p_random)
...
if __name__ == '__main__':
    try:
        if len(sys.argv) == 6 and sys.argv[1] == '-game':
            ...
            try:
                ...
                if hasattr(mcts, "TEST_REPEATS"):
                    count = count * mcts.TEST_REPEATS
            ...
            test_game(count, depth, p_random, minimax.seeds_stored_heuristic, n)
            sys.exit(0)
    except KalahParseError as err:
        print(sys.argv[0] + ":", str(err))
        sys.exit(1)
    
    print("USAGE: {python3 | pypy3}", sys.argv[0], "-game count depth prob-random n")
